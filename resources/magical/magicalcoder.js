...
McECharts.prototype.ajaxRequestData = function(param,remoteApi,callback){
    if(typeof axios!='undefined'){
        //用axios请求
        axios.get(remoteApi, {params: {}})
            .then(function (response) {
                var data = response.data;
                if(data.code == 0){//注意此处的数据协议
                    var dataList = data.data;
                    callback(param,dataList);
                }
            })
...
McECharts.prototype.line = function () {
    ...
    for(var i=0;i<charsDoms.length;i++){
        ...
        var remoteApi = userConfigOption.api;//数据源配置
        if(!remoteApi || !userConfigOption.x || !userConfigOption.y){
            remoteApi = "assets/drag/js/data/echarts/xy/test.json";//默认没数据时候的接口
        }
        //因为是循环并且异步 需要一直传参 保证在当前方法栈顶位置
        var param = {myEchart:myEchart,userConfigOption:userConfigOption}
        this.ajaxRequestData(param,remoteApi,function(param,dataList){
            userConfigOption = param.userConfigOption;
            myEchart = param.myEchart;
            var xFieldName = userConfigOption.x||'x';
            var yFieldName = userConfigOption.y||'y';
            ...
            for(var j=0;j<dataList.length;j++){
                ...
                seriasData.push(item[yFieldName]);
            }
            var option = {
                xAxis:{
                    type:"category",
            ...
            //把数据合并
            _t.combineSeries(option,userConfigOption);
            //重置echarts
...
McECharts.prototype.pie = function () {
    ...
    for(var i=0;i<charsDoms.length;i++){
        ...
        this.ajaxRequestData(param,remoteApi,function(param,dataList){
            ...
            for(var j=0;j<dataList.length;j++){
                var item = dataList[j];
                seriasData.push({value:item[yFieldName],name:item[xFieldName]});
            }
            var option = {
                ...
                series: [
                    {
                        ...
                        radius: '55%',
                        center: ['50%', '50%'],
                        data: seriasData.sort(function (a, b) { return a.value - b.value; }),
                        roseType: 'radius',
                        label: {
...
McECharts.prototype.scatter = function () {
    ...
    for(var i=0;i<charsDoms.length;i++){
        ...
        var remoteApi = userConfigOption.api;//数据源配置
        if(!remoteApi){
            remoteApi = "assets/drag/js/data/echarts/scatter/test.json";//默认没数据时候的接口
        }
        //因为是循环并且异步 需要一直传参 保证在当前方法栈顶位置
        var param = {myEchart:myEchart,userConfigOption:userConfigOption}
        this.ajaxRequestData(param,remoteApi,function(param,dataList){
